...
class CostumerTest {
    ...
    void testFindFirstCostumeWithKeyWord() {
        ...
        assertEquals("tűzokádó sárkány jelmez", costumer.findFirstCostumeWithKeyWord("sárkány").get().description());
        assertEquals("napocska jelmez", costumer.findFirstCostumeWithKeyWord("nap").get().description());
    }

    @Test
    void testFindFirstCheaperCostume() {
        assertEquals(Optional.empty(), costumer.findFirstCheaperCostume(100));
        assertEquals(2100, costumer.findFirstCheaperCostume(2400).get().price());
        assertEquals(2500, costumer.findFirstCheaperCostume(3000).get().price());
    }

    @Test
    void testFindFirstCostumeSameSize() {
        assertEquals(Optional.empty(), costumer.findFirstCostumeSameSize(Size.XL));
        assertEquals("űrhajós jelmez", costumer.findFirstCostumeSameSize(Size.M).get().description());
        assertEquals("zebra jelmez", costumer.findFirstCostumeSameSize(Size.L).get().description());
    }
}